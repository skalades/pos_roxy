...
import { Plus, AlertTriangle, ReceiptText, ArrowUpRight, Clock, User, Trash2, Inbox, X, ChevronRight } from 'lucide-react';
...
export default function ExpenseIndex({ expenses, active_shift }) {
    ...
    return (
        <AuthenticatedLayout
            header={
                <PageHeader 
                    ...
                    subtitle={`Total hari ini: ${formatIDR(totalExpense)}`}
                    badge="Biaya Operasional"
                    badgeColor="rose"
                    action={
                        <button
                            onClick={() => setShowAddModal(true)}
                            className="bg-roxy-primary hover:bg-teal-600 text-white px-8 py-4 rounded-[1.25rem] text-[11px] font-black transition-all shadow-xl shadow-roxy-primary/20 active:scale-95 flex items-center justify-center gap-3 uppercase tracking-widest"
                        >
        ...
        >
            ...
            <div className="max-w-5xl mx-auto space-y-6 landscape:space-y-4">
                {/* Active Shift Warning if not open */}
                {!active_shift && (
                    <div className="bg-rose-50 border-2 border-rose-100 p-6 rounded-[2.5rem] flex items-center gap-5 animate-pulse">
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <CurrencyInput
                                ...
                                className="focus:ring-roxy-primary/10 focus:border-roxy-primary"
                                required
                                isFocused
                            />
                        ...
                        <div className="pt-4">
                            <button
                                ...
                                className="w-full bg-slate-900 hover:bg-slate-800 text-white py-5 rounded-[2rem] text-lg font-black transition-all shadow-xl shadow-slate-900/20 active:scale-[0.98] flex items-center justify-center gap-3 uppercase tracking-widest disabled:opacity-50"
                            >
                                {processing ? (
            ...
            <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)} maxWidth="sm">
                <div className="p-8 text-center space-y-6">
                    <div className="w-20 h-20 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center mx-auto shadow-lg shadow-rose-500/10">
                        <Trash2 size={40} />
                    </div>
                    <div>
